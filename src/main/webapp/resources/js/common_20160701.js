...
/* 결재라인설정 height 사이즈*/
function ealineH(){
	// var sccon_hei = $(".sub_con").height();	
	var sl_hei=$(".sub_left").height();
$(".ova_line").height(sl_hei - 95);	
}
...
function subConWid(){
	...
	var k_wid=$(".k-splitbar").width();
	 if ($(".side_wrap").length > 0) 
	 {
		...
		/*퍼블리싱화면에서만 데이터테이블추가시 칼럼이 안맞기때문에 추가*/
...
/*레이어팝업***************************************************************************************************/    
        ...
        // - width : 팝업 가로사이즈
        // - height : 팝업 세로사이즈
        // - opener : 팝업호출페이지유형 (컨텐츠 : 1 / 윈도우팝업 : 2 / 레이어 : 3)
