...
function outProcessLogOn(params){
	...
	if(location.host.indexOf("127.0.0.1") > -1 || location.host.indexOf("localhost") > -1){
    	form.prop("action", "http://gw.ltikorea.or.kr/gw/outProcessLogOn.do");
    }else{
	...
	if(params.mod == "W"){
		...
		$.ajax({
			type : 'POST',
			async: false,
			url : '/CustomNPKlti/outProcess/outProcessTempInsert',
			data:data,
			dataType : 'json',
...
//					form.submit();
		...
		var url = form.prop("action") + "?compSeq=" + params.compSeq + "&empSeq=" + params.empSeq + "&approKey=" + params.approKey + "&outProcessCode=" + params.outProcessCode +"&mod=" + params.mod;
		win = window.open(url, "outProcessLogOn", "scrollbars=yes, width=1000px, height=900px, resizable=no, status=no, top=50, left=50", "newWindow");
//		form.submit();
	}
...
//	}
	...
	if(params.prevUrl){
		var prevUrl = $('<input type="hidden" name="prevUrl"/>');
		prevUrl.val(params.prevUrl);
		form.append(prevUrl);
		url += "&prevUrl="+params.prevUrl;
	}
	if(params.prevName){
		var prevName = $('<input type="hidden" name="prevName"/>');
		prevName.val(params.prevName);
		form.append(prevName);
		url += "&prevName="+params.prevName;
	}
	if(params.title){
...
function outProcessDocInterlockInsert(params){
	if(params.mod == "W"){
		...
		$.ajax({
			type : 'POST',
			async: false,
			url : '/CustomNPKlti/outProcess/outProcessDocInterlockInsert',
			data:data,
			dataType : 'json',
			success : function(data) {
				if(data.resultCode == "SUCCESS"){
