	...
	<div class="sub_contents_wrap">
		...
		<div class="top_box">
			<dl>
				...
				<dd>
					<input id="deptListBox" class="select-box" type="text" value="${userInfo.orgnztNm} / ${userInfo.empName}" disabled="disabled">
				...
				<dt class="ar" style="width: 65px">&nbsp;</dt>
				<dd class="controll_btn p0">
...
<!-- 			<div class="left_div">
		...
		<div class="btn_div mt10 cl">
			<div class="left_div">
				<p class="tit_p mt5 mb0">유연근무 상세내역</p>
			</div>
		</div>
		...
		<div class="btn_div mt10 cl">
			<div class="left_div">
				<p class="tit_p mt5 mb0">유연근무 변경 신청현황</p>
			</div>
			<div class="right_div">
				<div class="controll_btn p0">
					<input type="button" onclick="modCancelBtn();" style="float: right" value="변경신청 취소">
				</div>
			</div>
		</div>
		</div>
		<div class="com_ta2 mt20" id="grid3">
		</div>
...
var dataSource2 = new kendo.data.DataSource({
    ...
    transport: { 
        read:  {
            url: _g_contextPath_+'/workPlan/adminWorkPlanDetail2',
            dataType: "json",
            type: 'post'
        },
      	parameterMap: function(data, operation) {
        	data.userSeq = $('#empSeq').val();
        	data.statusType = $('#statusType').val();
        	data.month = $('#monthpicker').val().replace(/-/gi,"");
     	return data;
     	}
 ...
 var dataSource3 = new kendo.data.DataSource({
	    ...
	    transport: { 
	        read:  {
	            url: _g_contextPath_+'/workPlan/workPlanChangeAppList2',
	            dataType: "json",
	            type: 'post'
	        },
	      	parameterMap: function(data, operation) {
...
//	       			 $('#fn_approval').css({'display':'inline-block'});
 ...
 function mainGrid3(){
		...
		grid3.table.on("click", ".checkbox", selectRow);
		...
		function codeGridClick(){
			var rows = grid3.select();
			var record;
			rows.each(function(){
				record = grid3.dataItem($(this));
				console.log(record);
			});
		}
	}
...
function fn_workStep(row) {
	...
	var key2 = row.work_plan_detail_id;
	
// 	if (nowDate == 'future') {
// 		if ( step != '4' ){
			return '<input type="checkbox" id="over'+key+'" name='+key2+'/'+step+' class="k-checkbox checkbox"/><label for="over'+key+'" class="k-checkbox-label"></label>';
// 		}else {
// 			return '';
// 		}
...
$(document).ready(function() {
	console.log($('#empSeq').val())
	$('#empSeq').val('${userInfo.empSeq}');
	$('#userSeq2').val('${userInfo.empSeq}');
	console.log($('#empSeq').val())
		 ...
		 function modCancelBtn() {
				...
				if (ch.length < 1) {
					...
					$.each(ch, function(i,v){
						...
						tem['work_plan_detail_id'] = $(v).attr("name").substring(0,5);
						console.log(tem['work_plan_detail_id'])
						tem['work_plan_history_id'] = $(v).attr("id").substring(4);
						console.log(tem['work_plan_history_id'])
						data[index] = tem;
						
						if ( $(v).attr("name").substring(6) != '0') {
