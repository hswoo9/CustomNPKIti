...
function f_search(){	
	...
	form1.action = "${pageContext.request.contextPath}/trip/pop/tripPositionGroupPop";
	form1.submit();	
}
...
function f_confirm(){
	...
	eval("form.pgroup_seq_" + $("#costDiv").val()).value = $("#pgroupSeq").val();
	eval("form.pgroupname_" + $("#costDiv").val()).value = $("#selectname").val();	
	self.close();
}

function f_select(seq, title, type){
	$("#pgroupSeq").val(seq);
	$("#selectname").val(title);
	$("#pgroupTable > tbody > tr").removeClass("on");
...
<body>
	<div class="pop_wrap">
		<div class="pop_head">
			<h1><%=DJMessage.getMessage("TX000020764","직책그룹", langCode)%> <%=DJMessage.getMessage("TX000003650","선택", langCode)%><c:if test="${domesticDiv == 'L' }">(<%=DJMessage.getMessage("TX000015728","국내", langCode)%>)</c:if><c:if test="${domesticDiv == 'F' }">(<%=DJMessage.getMessage("TX000020684","국외", langCode)%>)</c:if></h1>
			<a href="#n" class="clo"><img src="${pageContext.request.contextPath}/Images/btn/btn_pop_clo02.png" alt="" /></a>
		</div>			
			
		<div class="pop_con">
			<div class="top_box">
			<form name="searchForm" id="searchForm" method="post" action="${pageContext.request.contextPath}/trip/pop/tripPositionGroupPop">
				...
				<input type="hidden" id="pgroupSeq" name="pgroupSeq" />				
				<dl>
			...
			<div class="com_ta2 sc_head mt15">
				<table>
					...
					<thead>
						<tr>
							<th>no</th>
							<th><%=DJMessage.getMessage("TX000018431","그룹명", langCode)%></th>
			...
			<div class="com_ta2 ova_sc2 cursor_p bg_lightgray" style="height:333px;">
				<table id="pgroupTable">
					...
					<tbody>					
						<c:choose>			
							<c:when test="${not empty tripPositionGroupListPop}">
								<c:forEach items="${tripPositionGroupListPop}" var="list" varStatus="status">
									<tr id="row_${list.pgroupSeq}" onclick="f_select('${list.pgroupSeq}', '${list.pgroupname}', 'one')" ondblclick="f_select('${list.pgroupSeq}', '${list.pgroupname}', 'dbl')"><!-- 선택 로우 클래스 on -->	
										<td>${status.count}</td>
										<td>${list.pgroupname}</td>
